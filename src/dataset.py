...
class CataractPhaseDataset(Dataset):
    ...
    with start/end frames for each phase. This version is modified to
    include only these phases:
    ...
    def __init__(self, root_dir, transform=None, frame_skip=10):
        ...
        # Build a sorted list of phase names (only the 4 we allow)
    ...
    def _read_phase_csv(self, case_id_str, csv_path):
        ...
        Only adds phases that are in self.allowed_phases.
        """
        ...
        with open(csv_path, "r", newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                # Skip phases that are not in our allowed list
                if phase_name not in self.allowed_phases:
                    continue
    ...
    def _build_samples(self):
        ...
        Skips frames that exceed total_frames, and uses frame_skip to reduce sampling.
        """
    ...
    def __getitem__(self, idx):
        ...
        if not ret or frame_bgr is None:
            # If there's an error reading the frame, just create a dummy black image.
            frame_bgr = np.zeros((224,224,3), dtype=np.uint8)

        frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)

        if self.transform:
            # If we're using e.g. albumentations, we might do: transform(image=frame_rgb)
            transformed = self.transform(image=frame_rgb)
            frame_tensor = transformed["image"]
        else:
            # Simple PyTorch tensor transform
